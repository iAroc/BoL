...
function SxOrbRaw:__init()
	self:LoadTables()
	self.SxOrbSettings = { ["ForceSelector"] = false, ["TargetRange"] = myHero.range + hitboxes[myHero.charName], ["RangeChanged"] = false, ["LastHitRange"] = myHero.range + hitboxes[myHero.charName]}
...
function SxOrbRaw:LoadToMenu(MainMenu, NoMenuKeys)
	...
	self.SxOrbMenu.generalsettings:addParam("StopMove", self:GetText("IDLE"), SCRIPT_PARAM_ONOFF, false)
...
function SxOrbRaw:OnDraw()
	if self.SxOrbMenu.drawsettings.AARange then
		self:CircleDraw(myHero.x, myHero.y, myHero.z, self.SxOrbSettings["LastHitRange"] + 30, 4294967295)
	end
...
function SxOrbRaw:ValidTarget(Target,AutoCarry)
	if AutoCarry then MyRange = self.SxOrbSettings["TargetRange"] + (hitboxes[Target.charName] or 0) else MyRange = self.SxOrbSettings["LastHitRange"] + (hitboxes[Target.charName] or 0) end
