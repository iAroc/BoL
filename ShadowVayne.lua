...
function OnTick()
	if not ScriptStartOver and DownLoadCount == 0 then
		...
		_RequireWithoutUpdate("Selector")
...
function _LoadMenu()
	...
	SVMainMenu:addSubMenu("[Misc]: TS Settings", "tssetting")
	...
	SVMainMenu.tssetting:addParam("UsedTS", "Choose your TargetSelector (need Reload!):", SCRIPT_PARAM_LIST, 1, {"Simple Target Selector", "VIP Target Selector"})
	if VIP_USER and SVMainMenu.tssetting.UsedTS == 2 then UserVIPSelector = true end
	for i, enemy in ipairs(GetEnemyHeroes()) do
...
--~ 		SVMainMenu.vip:addParam("selector", "Use Selector (VIP Only)", SCRIPT_PARAM_ONOFF, true)
